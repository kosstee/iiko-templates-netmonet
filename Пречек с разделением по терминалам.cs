...
@{
    ...
    var terminalDictionary = new Dictionary <string, Tuple <string, string>> {
        // указать имя группы
        {"Terminal 1", Tuple.Create("XXXXXX", "&wpid=XXXXXX")}, // изменить XXXXXX на соответствующие предоставленные коды заведения
        {"Terminal 2", Tuple.Create("XXXXXX", "&wpid=XXXXXX")}
        };
...
<doc>
    ...
    @* Footer (begin) *@
    @if (terminalDictionary.ContainsKey(terminal)) {
        if (codeFound) {
            <f2><center>@("Отзывы и чаевые")</center></f2>
            <f2><center>@("нетмонет")</center></f2>
            <center>@("Наведите камеру на QR-код")</center>
            <qrcode size="small" correction="low">https://netmonet.co/tip/@code?o=@urls["1"]=@fullSum@urls["2"]=@order.Number@urls["5"]=@order.Table.Number@terminalDictionary[@terminal].Item2</qrcode>
            <center>@("или введите " + @code + " на netmonet.co")</center>
            } else {
                <f2><center>@("Отзывы и чаевые")</center></f2>
                <f2><center>@("нетмонет")</center></f2>
                <center>@("Наведите камеру на QR-код")</center>
                <qrcode size="small" correction="low">https://netmonet.co/tip/@terminalDictionary[@terminal].Item1?o=@urls["1"]=@fullSum@urls["2"]=@order.Number@urls["5"]=@order.Table.Number@urls["3"]=@transliteratedName</qrcode>
                <center>@("или введите " + @terminalDictionary[@terminal].Item1 + " на netmonet.co")</center>
                }
    }
