...
@{
    ...
    var waiter = Model.Order.Waiter.GetNameOrEmpty();
    var waitersDictionary = new Dictionary<string, string> {
        {"Официант 1", "XXXXXX"},
        {"Официант 2", "XXXXXX"},
        {"Официант 3", "XXXXXX"},
        {"Официант 4", "XXXXXX"},
...
<doc>
    ...
    @* Footer (begin) *@
    <np />
    @if (waitersDictionary.ContainsKey(waiter)) {
        <f2><center>@("Отзывы и чаевые")</center></f2>
        <f2><center>@("нетмонет")</center></f2>
        <center>@("Наведите камеру на QR-код")</center>
        <qrcode size="small" correction="low">https://netmonet.co/tip/@waitersDictionary[waiter]?o=@urls["1"]=@fullSum@urls["2"]=@order.Number@urls["5"]=@order.Table.Number@urls["6"]</qrcode>
        <center>@("или введите " + @waitersDictionary[waiter] + " на netmonet.co")</center>
        } else {
            <f2><center>@("Отзывы и чаевые")</center></f2>
            <f2><center>@("нетмонет")</center></f2>           
            <center>@("Наведите камеру на QR-код")</center>
